...
def facepull(video_path):
    ...
    while True:
        ...
        if len(lmList) != 0:
            ...
            per_left_shoulder = np.interp(angle_left_side_shoulder, (265, 170), (0, 100))
            per_left_elbow = np.interp(angle_left_side_elbow, (165, 220), (0, 100))
            # check for pullups
            if ((per_left_elbow >= 50) and (per_left_shoulder >= 50)) and attempted_dir == 0:
                attempted_reps += 0.5
                attempted_dir = 1
            if ((per_left_eblow < 50) and (per_left_shoulder < 50)) and attempted_dir == 1:
                attempted_reps += 0.5
                attempted_dir = 0
            #person reached the bottom of the rep
            if per_left_elbow == 100 and per_left_shoulder == 100 and (170 <= angle_left_side_back <= 190):
                if dir == 0:
                    count += 0.5
                    dir = 1
            ...
            if per_left_elbow == 0 and per_left_shoulder == 0 and (170 <= angle_left_side_back <= 190):
                if dir == 1:
                    count += 0.5
                    dir = 0
