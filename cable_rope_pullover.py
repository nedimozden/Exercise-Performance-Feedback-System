...
def cable_rope_pullover(video_path):
    ...
    while True:
        ...
        if len(lmList) != 0:
            ...
            angle_left_side_back = detector.findAngle(img, 12, 24, 26)
            per_left_elbow = np.interp(angle_left_side_elbow, (155, 115), (0, 100))
            per_left_shoulder = np.interp(angle_left_side_shoulder, (250, 30), (0, 100))
            per_left_back = np.interp(angle_left_side_back, (120, 160), (0, 100))

            # check for pullups
            if ((per_left_elbow >= 50) and (per_left_shoulder >= 50) and (per_left_back >= 50)) and attempted_dir == 0:
                attempted_reps += 0.5
                attempted_dir = 1
            if ((per_left_elbow < 50) and (per_left_shoulder < 50) and (per_left_back < 50)) and attempted_dir == 1:
                attempted_reps += 0.5
                attempted_dir = 0
            # person reached the bottom of the rep
            if per_left_elbow == 100 and per_left_shoulder == 100 and per_left_back == 100:
                if dir == 0:
                    count += 0.5
                    dir = 1
            ...
            if per_left_elbow == 0 and per_left_shoulder == 0 and per_left_back == 0:
                if dir == 1:
                    count += 0.5
                    dir = 0
